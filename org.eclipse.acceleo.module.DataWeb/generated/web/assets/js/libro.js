...
class LibroManager {
    ...
    async createEntity() {
        ...
        if (data.añoPublicacion) data.añoPublicacion = parseInt(data.añoPublicacion) || 0;
        data.disponible = data.disponible === 'true' || data.disponible === true;
        
        try {
            await this.db.create(data);
            this.showMessage('Libro creado correctamente', 'success');
            setTimeout(() => {
                window.location.href = 'list.html';
            }, 1500);
        } catch (error) {
            console.error('Error creating entity:', error);
            this.showMessage('Error al crear Libro', 'error');
        }
    }
    ...
    async updateEntity() {
        ...
        // Convertir tipos de datos
        if (data.id_libro) data.id_libro = parseInt(data.id_libro) || 0;
        if (data.añoPublicacion) data.añoPublicacion = parseInt(data.añoPublicacion) || 0;
        data.disponible = data.disponible === 'true' || data.disponible === true;
        ...
        try {
            ...
            this.showMessage('Error al actualizar Libro', 'error');
        }
    }
    ...
    async loadTableData() {
        try {
            ...
            tbody.innerHTML = entities.map(entity => `
                <tr>
                    <td>${this.formatValue(entity.id_libro, 'INTEGER')}</td>
                    <td>${this.formatValue(entity.titulo, 'STRING')}</td>
                    <td>${this.formatValue(entity.autor, 'STRING')}</td>
                    <td>${this.formatValue(entity.añoPublicacion, 'INTEGER')}</td>
                    <td>${this.formatValue(entity.disponible, 'BOOLEAN')}</td>
                    <td>
    ...
    async loadEntityForEdit() {
        ...
        try {
            ...
            if (entity) {
                ...
                if (field_disponible) {
                    field_disponible.checked = Boolean(entity.disponible);
                }
    ...
    async loadEntityDetails() {
        ...
        try {
            ...
            if (entity) {
                ...
                if (field_id_libro) {
                    field_id_libro.textContent = this.formatValue(entity.id_libro, 'INTEGER');
                }
                const field_titulo = document.getElementById('field_titulo');
                if (field_titulo) {
                    field_titulo.textContent = this.formatValue(entity.titulo, 'STRING');
                }
                const field_autor = document.getElementById('field_autor');
                if (field_autor) {
                    field_autor.textContent = this.formatValue(entity.autor, 'STRING');
                }
                const field_añoPublicacion = document.getElementById('field_añoPublicacion');
                if (field_añoPublicacion) {
                    field_añoPublicacion.textContent = this.formatValue(entity.añoPublicacion, 'INTEGER');
                }
                const field_disponible = document.getElementById('field_disponible');
                if (field_disponible) {
                    field_disponible.textContent = this.formatValue(entity.disponible, 'BOOLEAN');
                }
    ...
    async deleteEntity(id) {
        ...
        try {
            ...
            this.showMessage('Error al eliminar Libro', 'error');
        }
    }
