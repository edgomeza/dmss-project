...
class CategoriaManager {
    ...
    async createEntity() {
        ...
        try {
            ...
            this.showMessage('Categoria creado correctamente', 'success');
            setTimeout(() => {
                window.location.href = 'list.html';
            }, 1500);
        } catch (error) {
            console.error('Error creating entity:', error);
            this.showMessage('Error al crear Categoria', 'error');
        }
    }
    ...
    async updateEntity() {
        ...
        // Convertir tipos de datos
        if (data.id_categoria) data.id_categoria = parseInt(data.id_categoria) || 0;
        
        try {
            await this.db.update(data);
            this.showMessage('Categoria actualizado correctamente', 'success');
            setTimeout(() => {
                window.location.href = 'list.html';
            }, 1500);
        } catch (error) {
            console.error('Error updating entity:', error);
            this.showMessage('Error al actualizar Categoria', 'error');
        }
    }
    ...
    async loadTableData() {
        try {
            ...
            tbody.innerHTML = entities.map(entity => `
                <tr>
                    <td>${this.formatValue(entity.id_categoria, 'INTEGER')}</td>
                    <td>${this.formatValue(entity.nombre_categoria, 'STRING')}</td>
    ...
    async loadEntityDetails() {
        ...
        try {
            ...
            if (entity) {
                ...
                if (field_id_categoria) {
                    field_id_categoria.textContent = this.formatValue(entity.id_categoria, 'INTEGER');
                }
                const field_nombre_categoria = document.getElementById('field_nombre_categoria');
                if (field_nombre_categoria) {
                    field_nombre_categoria.textContent = this.formatValue(entity.nombre_categoria, 'STRING');
                }
    ...
    async deleteEntity(id) {
        ...
        try {
            ...
            this.showMessage('Categoria eliminado correctamente', 'success');
            this.loadTableData();
        } catch (error) {
            console.error('Error deleting entity:', error);
            this.showMessage('Error al eliminar Categoria', 'error');
        }
    }
