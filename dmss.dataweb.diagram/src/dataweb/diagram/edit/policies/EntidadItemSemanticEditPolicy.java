...
import dataweb.diagram.edit.commands.ReferenciaCreateCommand;
import dataweb.diagram.edit.commands.ReferenciaReorientCommand;
...
public class EntidadItemSemanticEditPolicy extends DatawebBaseItemSemanticEditPolicy {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (DatawebVisualIDRegistry.getVisualID(incomingLink) == ReferenciaEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(incomingLink.getElement(), false);
				cmd.add(new DestroyElementCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), incomingLink));
				continue;
			}
			if (DatawebVisualIDRegistry.getVisualID(incomingLink) == PaginaCRUDEntidadEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (DatawebVisualIDRegistry.getVisualID(outgoingLink) == ReferenciaEditPart.VISUAL_ID) {
	...
	*/
					...
					case AtributoEditPart.VISUAL_ID:
						for (Iterator<?> it = cnode.getTargetEdges().iterator(); it.hasNext();) {
							...
							if (DatawebVisualIDRegistry
									.getVisualID(incomingLink) == PaginaIndiceAtributosMostradosEditPart.VISUAL_ID) {
							...
							if (DatawebVisualIDRegistry.getVisualID(
									incomingLink) == PaginaIndiceFiltrosAtributosFiltroEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(), null,
										incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r));
							...
							if (DatawebVisualIDRegistry.getVisualID(
									incomingLink) == PaginaIndicePaginadaFiltradaAtributosFiltroEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
										incomingLink.getSource().getElement(), null,
										incomingLink.getTarget().getElement(), false);
								cmd.add(new DestroyReferenceCommand(r));
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (DatawebElementTypes.Referencia_4001 == req.getElementType()) {
			return getGEFWrapper(new ReferenciaCreateCommand(req, req.getSource(), req.getTarget()));
		}
	...
	protected Command getReorientRelationshipCommand(ReorientRelationshipRequest req) {
		...
		case ReferenciaEditPart.VISUAL_ID:
			return getGEFWrapper(new ReferenciaReorientCommand(req));
		}
